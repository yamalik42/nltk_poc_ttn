...
from MatchRecord.settings import MOCK_DB, MOCK_FORM_DATA, NON_SUBJECTIVE_KEYS
import nltk
...
def nlp_view(request):
    ...
    if request.method == 'POST':
        clean_data = {k: v for k, v in request.POST.items() if v != ''}
        del clean_data['csrfmiddlewaretoken']
        possible_duplicates = get_similar_projects(**clean_dat)

    elif request.method == 'GET':
        form_tags = get_tagged_tokens(MOCK_FORM_DATA[0])
        form_stems = list(dict.fromkeys(get_stems(get_lemmas(form_tags))))

        duplicates = get_possible_duplicates()
        for duplicate in duplicates:
            dup_tags = get_tagged_tokens(duplicate)
            dup_stems = list(dict.fromkeys(get_stems(get_lemmas(dup_tags))))
            import pdb;pdb.set_trace()
            similarity = calc_similarity(form_stems, dup_stems)
            import pdb;pdb.set_trace()
...
def get_possible_duplicates():
    ...
    for record in MOCK_DB:
        ...
        for key in NON_SUBJECTIVE_KEYS:
            if record[key] != MOCK_FORM_DATA[0][key]:
...
def calc_similarity(new_entry, old_entry, intersect_count=0):
    ...
    for stem in order_by_len[0]:
        if stem in order_by_len[1]:
            intersect_count += 1
    import pdb;pdb.set_trace()
    union_count = len(new_entry) + len(old_entry) - intersect_count
